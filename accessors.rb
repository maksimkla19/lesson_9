  ...
  module ClassMethods
      def attr_accessor_with_history(*methods)
          methods.each do |method|
              ...
              define_metods("#{method}_history") {instance_variable_get(history_method)}
              define_methods("#{method}=") do |value|
                  if instance_variable_get(history_method).nil?
                   ...
                   def strong_attr_acessor(name, type)
                     name = "@#{name}"
                     define_metods(name) {instance_variable_get(name)}
                     define_metods("@#{name}=") do |value|
                       if value.is_a?(type)
